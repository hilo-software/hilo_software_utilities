...
from hilo_software_utilities.send_mail import send, send_text_email, send_file_email
...
def test_send_success(mock_smtp, dummy_email_msg):
    send("test@example.com", "test@example.com", "dummy_app_key", dummy_email_msg)
...
@patch("hilo_software_utilities.send_mail.send_mail.smtplib.SMTP", side_effect=smtplib.SMTPException("SMTP failed"))
def test_send_text_email(mock_send):
    send_text_email("test@example.com", "dummy_app_key", "Test Subject", "This is a test.")
...
def test_send_file_email_success(mock_send_text, tmp_path):
    ...
    send_file_email("test@example.com", "dummy_app_key", "Log Subject", str(test_file))
    mock_send_text.assert_called_once()


def test_send_file_email_failure(tmp_path, caplog):
    bad_file = tmp_path / "nonexistent.txt"
    send_file_email("test@example.com", "dummy_app_key", "Log Subject", str(bad_file))
